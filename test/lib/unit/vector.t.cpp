...
TEST_CASE("vector: test05: operators on vectors from different libraries",
          "[vector]")
{

  auto const tolerance = std::numeric_limits<double>::epsilon();
  using vector = math::vector<math::serial_cpu_engine>;
  using lac_vector = lac::vector<math::serial_cpu_engine>;
